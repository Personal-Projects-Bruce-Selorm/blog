...
util.rateLimit = function (phone, callback) {
    ...
    data.read('route-force-tracker', phone, function (dataObject) {
        if (dataObject) {
            //check count
            if (dataObject.count < 5) {
                ...
                data.update('route-force-tracker', phone, dataObject, function (bool) {
                    if (bool) {
                        callback(false, { Success: "user tracker updated" })

                    } else {
                        callback(true, { Error: "updateFail" })
                    }

                })

            } else {
                var correlateAllTimes = 0;
                //check total number  attempts in a minute
                dataObject.attempsTimeStamps.forEach((value) => {
                    correlateAllTimes += value;
                })
                if ((correlateAllTimes / 5) <= (Date.now() - 1000 * 60)) {
                    //five login attempt on same user in less than a minute
                    if (dataObject.nextAttempt <= Date.now()) {
                        callback(true, { Error: "please note that due to many attempt in a short time, your account is deactivated for the next one hour" })
                    } else {
                        dataObject.nextAttempt = Date.now() + (1000 * 60 * 60)
                        dataObject.count = 1;
                        dataObject.attempsTimeStamps = [Date.now()];
                        data.update('route-force-tracker', phone, dataObject, function (bool) {
                            if (bool) {
                                callback(false, { Success: "user tracker next Attempt set" })
                    ...
                    data.update('route-force-tracker', phone, trackerObject, function (bool) {
                        if (bool) {
                            callback(false, { Success: "user tracker refreshed" })
            ...
            data.create('route-force-tracker', phone, trackerObject, function (bool) {
                if (bool) {
                    callback(false, { Success: "tracker set on user" })
                } else {
                    callback(true, { Error: "could not create route-force tracker for this user" })
                }
            })
        }
    })
